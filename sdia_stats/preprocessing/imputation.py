# -*- coding: utf-8 -*-
"""
Created on Tue Apr 23 08:41:36 2024
...
def plot_histograms(original, imputed, title, metadata):
    """
    Plot histograms comparing imputed and unimputed values.
    
    Parameters:
    - annotated (pd.DataFrame): DataFrame after imputation.
    - original (pd.DataFrame): Original DataFrame before imputation.
    - title (str): Title for the histograms.
    - metadata (pd.DataFrame): Metadata to subset data further if needed.
    """
    # Mask for original NaNs
    nan_mask = original.isna()
    # ic(nan_mask)
    
    # Create two subsets for imputed and unimputed values using the mask
    imputed_vals = imputed.where(nan_mask)
    unimputed_vals = imputed.where(~nan_mask)
    
    # Assuming the metadata helps in subsetting further
    # Ensure your metadata handling logic is correctly implemented here if needed
    relevant_columns = ['Protein.Group'] + metadata['Sample'].tolist()
    imputed_vals = imputed_vals[relevant_columns]
    unimputed_vals = unimputed_vals[relevant_columns]

    # Plot histograms for each column
    for col in imputed_vals.columns[1:]:  # Assuming first column is 'Protein.Group'
        plt.figure(figsize=(10, 6))
        sns.histplot(unimputed_vals[col].dropna(), color='orange', label='Original', alpha=0.6, bins=300)
        sns.histplot(imputed_vals[col].dropna(), color='blue', label='Imputed', alpha=0.6, bins=300)
...
def perform_imputation(path, group, meta):
    ...
    print('Filter for valid vals')
    light_filt = filter_valid_values(light, metadata)
    nsp_filt = filter_valid_values(nsp, metadata)
    
    unimputed_light = light_filt.copy(deep=True)
    unimputed_nsp = nsp_filt.copy(deep=True)
    ...
    light_annotated = annotate_df(light_filt, metadata, 'light', control_samples)
    nsp_annotated = annotate_df(nsp_filt, metadata, 'nsp', control_samples)
    ...
    if plot_imputation:
        plot_histograms(unimputed_light, light, 'Light', metadata)
        plot_histograms(unimputed_nsp, nsp, 'Nsp', metadata)
...
#               }
# perform_imputation(path, group, meta)
